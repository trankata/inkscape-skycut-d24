...
def overcut_along_path(pts, ov_dist):
    """Overcut по дъгата — връща точки до ov_dist разстояние от pts[0]."""
    work = list(pts)
...
def emit_path(hpgl, pts, is_closed, is_curved, k_off, ov_mm, tk_mm,
    ...
    """Генерира HPGL команди за един P1 затворен контур."""

    base_start = pts[0]
    phase1 = list(pts)

    # Компенсация за затварящия ъгъл
    ...
    # Pre-start knife orientation (само за прави)
...
class SkyCutUnified(inkex.EffectExtension):
    ...
    def effect(self):
        ...
        if debug:
            inkex.errormsg(f"DEBUG: use_markers={use_markers}, "
        ...
        if use_markers:
            ...
            # Без маркери — координатна система от bounding box
            path_data_tmp = process_elements(cut_layer, scale_x, scale_y)
            if not path_data_tmp:
                inkex.errormsg("No cut paths found"); return
            all_x = [p[0] for item in path_data_tmp for p in item['pts']]
            all_y = [p[1] for item in path_data_tmp for p in item['pts']]
        ...
        # ── Пътища ───────────────────────────────────────────────────────────
        path_data = process_elements(cut_layer, scale_x, scale_y)
        if not path_data:
            inkex.errormsg("No cut paths found"); return
        ...
        for item in path_data:
            ...
            # base_cycle без closing duplicate
            if is_closed:
