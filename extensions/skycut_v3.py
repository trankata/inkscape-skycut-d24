...
def _stroke_to_tool(elem):
    ...
    if any(c in color for c in ('#000000', 'black', '#000', 'rgb(0,0,0)')):
        return "P0", 0
    if any(c in color for c in ('#ff0000', 'red', '#f00', 'rgb(255,0,0)')):
        return "P1", 2
    return "P1", 1
...
class SkyCutV3(inkex.EffectExtension):
    ...
    def _build_hpgl(self):
        ...
        all_paths = process_elements(cut_layer, scale, scale)
        if not all_paths:
