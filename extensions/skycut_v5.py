...
def emit_dashed_path(hpgl, pts, coord, dash_mm, gap_mm,
    ...
    """Емитира пунктир: върви по pts, редува dash (рязано) и gap (пауза).

    dash_mm / gap_mm  — дължини на рязания сегмент и паузата
    dash_fs / gap_fs  — натиск за dash / gap (None = ползвай base_fs)
    cut_quickly       — True: ножът остава долу, само сменя FS
                        False: вдига ножа (U) в паузите
    base_fs           — основният натиск на цвета (когато dash/gap_fs са None)

    Поведение възпроизведено от оригиналния плъгин:
      cut_quickly + forces:  FS<dash>;D..D.. FS<gap>;D..D.. (нож долу)
      cut_quickly без forces: само base_fs, но пак с U в паузите
      без cut_quickly:        вдига ножа (U) в началото на всеки gap
    """
    pts = dedup_pts(pts)
    pts = collinear_clean(pts)
    if len(pts) < 2:
    ...
    # Върви по полилинията, отчитайки изминато разстояние
    in_dash = True          # започваме с рязан сегмент
    remaining = dash_mm     # оставаща дължина в текущото състояние
    pen_down = False
    ...
    # Започни първия dash: спусни ножа
    ...
    while i < len(pts):
        ...
        if seg <= remaining:
            # целият сегмент е в текущото състояние
            if in_dash:
                move_to(nx, ny, True)
            else:
                # gap: ако cut_quickly → лек натиск (нож долу), иначе вдигнат
            ...
            if remaining <= 1e-6:
                ...
                if in_dash:
                    if cut_quickly:
                        ...
                        # вдигнатият нож трябва да слезе за новия dash
                        move_to(cx, cy, True)
                else:
                    if cut_quickly:
                        if cur_fs != gf:
                        ...
                        # започва пауза → вдигни ножа на текущата точка
                        move_to(cx, cy, False)
        else:
            # сегментът пресича границата — раздели го
            t = remaining / seg
            mx = cx + (nx-cx)*t
            my = cy + (ny-cy)*t
            if in_dash:
...
def _simple_tool(elem):
    """Прост режим (като v3): black→P0 (биг), друго→P1 (рязане)."""
...
def process_elements(cut_layer, color_settings, scale_x=1.0, scale_y=1.0):
    """Ако color_settings е dict → цветови режим (tool/force/speed/seq по цвят).
    Ако color_settings е None → прост режим (black=P0, друго=P1, без FS/VS)."""
    path_data = []
    for elem in cut_layer.iterdescendants():
        ...
        for subpath in csp:
            ...
            # Определи затвореност: ZoneClose в пътя, или близост начало/край
...
class SkyCutV5(inkex.EffectExtension):
    ...
    def add_arguments(self, pars):
        ...
        pars.add_argument("--output_path",   type=str,           default="skycut_v5_output.hpgl")
    ...
    def _build_hpgl(self):
        ...
        # Emit paths.
        # Цветови режим: преди всеки блок с нови настройки → P;FS;VS
        # Прост режим: само P при смяна на инструмента (като v3)
        current_key = None
        for item in final_sequence:
            ...
            # Подготви точките (затворен → отваряме + knife offset + overcut;
            # отворен → knife offset). После, ако е пунктир, режем dashed.
            if is_closed:
            ...
            if is_dashed:
                # Пунктир: US travel speed + разбиване dash/gap
